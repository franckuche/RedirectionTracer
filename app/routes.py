from fastapi import APIRouter, Request, File, Form, UploadFile, BackgroundTasks, HTTPException
...
async def filter_results(request: Request, task_id: str, status: str = "all", http_code: Optional[str] = None):
    ...
    try:
        ...
        return HTMLResponse(
            content=f"<tr><td colspan='6' class='px-6 py-8 text-center text-danger-500 border-b border-secondary-200'>"
            f"<p class='text-lg font-medium'>Erreur: {str(e)}</p></td></tr>"
        )

@router.get("/api/task/{task_id}/export")
async def export_results(request: Request, task_id: str, format: str = "csv", status: str = "all", http_code: Optional[str] = None, search: Optional[str] = None):
    """Exporter les résultats filtrés au format CSV"""
    # Vérifier si la tâche existe
    if task_id not in tasks:
    ...
    # Récupérer les résultats
    if "details" not in task_info or not task_info["details"]:
    ...
    for detail in details_list:
        # Filtre par statut
        status_match = status.lower() == "all" or detail.status.lower() == status.lower()
        ...
        if search and search.strip():
            ...
            search_match = (
                search_term in detail.source.lower() or
                search_term in detail.target.lower() or
                search_term in detail.final.lower() or
                search_term in detail.conclusion.lower()
            )
        
        # Appliquer tous les filtres (ET logique)
        if status_match and http_code_match and search_match:
    ...
    # Écrire l'en-tête
    writer.writerow(["Ligne", "URL Source", "URL Cible", "URL Finale", "Code HTTP", "Conclusion", "Statut", "Redirections"])
    ...
    for detail in filtered_details:
        writer.writerow([
            ...
            detail.conclusion,
            detail.status,
            getattr(detail, "redirections_count", 0)
        ])
    
    # Préparer la réponse
    response = Response(content=output.getvalue())
    response.headers["Content-Disposition"] = f"attachment; filename=redirections_{task_id}.csv"
    response.headers["Content-Type"] = "text/csv"
    
    return response

@router.get("/api/task/{task_id}/download")
async def download_results(task_id: str, filtered: bool = False):
    """Télécharger les résultats d'une tâche au format CSV (global ou filtré)"""
    # Vérifier si la tâche existe
    if task_id not in tasks:
...
async def download_filtered_results(task_id: str):
